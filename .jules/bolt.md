...
## 2024-07-28 - [Batching NVS Fetches]
**Learning:** ESP-IDF NVS implementation incurs I/O latency and unnecessary flash wear when opening and closing handles for every parameter read. In `app_main` and handler initialization, sequential calls to `get_config_param_str` and `get_config_param_int` were triggering >10 open/close cycles, creating a bottleneck.
**Action:** When multiple NVS parameters are retrieved sequentially, avoid the N+1 anti-pattern. Expose `nvs_get_*` functions that accept an open `nvs_handle_t` to fetch parameters in a batch within a single `nvs_open`/`nvs_close` block to optimize boot time.
