...
## 2024-11-20 - [Denial of Service via Heap Corruption]
**Vulnerability:** A Denial of Service (DoS) vulnerability existed in `advancedhandler.c` where a pointer initialized to a string literal (`char *currentDNS = "";`) was unconditionally freed (`free(currentDNS);`) if a DNS retrieval function (`esp_netif_get_dns_info`) failed, leading to heap corruption and crashing the router.
**Learning:** Initializing pointers to string literals and conditionally reassigning them to dynamically allocated memory is extremely dangerous if the pointer is unconditionally freed later. If the condition is false, `free()` attempts to free the read-only memory segment of the string literal, causing a crash.
**Prevention:** Always initialize such pointers to `NULL` (since `free(NULL)` is a safe no-op) or dynamically allocate them upfront (e.g., `currentDNS = malloc(16); currentDNS[0] = '\0';`) to ensure the pointer is always safe to free.
