...
void fillMac()
{
    ...
    if (customMac != NULL)
    {
        if (strcmp("random", customMac) == 0)
        {
            ...
            ESP_LOGI(TAG, "Setting random MAC address: %02x:%02x:%02x:%02x:%02x:%02x", default_mac_addr[0], default_mac_addr[1], default_mac_addr[2], default_mac_addr[3], default_mac_addr[4], default_mac_addr[5]);
