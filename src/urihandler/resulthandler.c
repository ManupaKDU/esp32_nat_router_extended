...
esp_err_t result_download_get_handler(httpd_req_t *req)
{
    ...
    ssize_t send_len = (response_len > 0 && response_len <= size) ? response_len : HTTPD_RESP_USE_STRLEN;
    esp_err_t ret = httpd_resp_send(req, result_page, send_len);
    ESP_LOGI(TAG, "Requesting result page with  %d additional bytes", (int)(send_len == HTTPD_RESP_USE_STRLEN ? strlen(result_page) : send_len));
