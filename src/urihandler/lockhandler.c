...
esp_err_t unlock_handler(httpd_req_t *req)
{
    ...
    if (fill_post_buffer(req, buf, content_len) == ESP_OK)
    {
        ...
        char unlockParam[param_len]; // ⚡ Bolt: Allocate on stack to avoid malloc overhead and heap fragmentation
...
esp_err_t lock_handler(httpd_req_t *req)
{
    ...
    if (req->method == HTTP_POST) // Relock if called
    {
        ...
        size_t param_len = 600;
        char passParam[param_len]; // ⚡ Bolt: Allocate on stack to avoid malloc overhead and heap fragmentation
        char pass2Param[param_len];
