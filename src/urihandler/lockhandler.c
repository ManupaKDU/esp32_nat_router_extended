    ...
    if (fill_post_buffer(req, buf, content_len) == ESP_OK)
    {
        ...
        if (strlen(unlockParam) > 0)
        {
            ...
            if (lock != NULL)
            {
                ...
                ESP_LOGW(TAG, "Invalid unlock password attempt");
                free(lock);
            }
            else
            {
                 ESP_LOGW(TAG, "Invalid unlock password attempt (no password set)");
            }
        }
        else
        {
            ESP_LOGW(TAG, "Empty unlock parameter received");
        }
        free(unlockParam);
    }
    free(buf);
...
esp_err_t lock_handler(httpd_req_t *req)
{
    ...
    if (req->method == HTTP_POST) // Relock if called
    {
        ...
        {
            ESP_LOGW(TAG, "Pass parameters do not match or are invalid");
        }
        free(passParam);
        free(pass2Param);
        free(buf);
