...
esp_err_t unlock_handler(httpd_req_t *req)
{
    ...
    closeHeader(req);
    // ⚡ Bolt: Eliminate O(N) strlen() overhead on static file
    return httpd_resp_send(req, ul_start, (size_t)(ul_end - ul_start) - 1);
}
...
esp_err_t lock_handler(httpd_req_t *req)
{
    ...
    // ⚡ Bolt: Capture sprintf length instead of relying on HTTPD_RESP_USE_STRLEN
