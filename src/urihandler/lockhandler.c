...
esp_err_t lock_handler(httpd_req_t *req)
{
    ...
    size_t lock_page_size = l_html_size + strlen(display) + 1;
    char *lock_page = malloc(lock_page_size);

    int len = snprintf(lock_page, lock_page_size, l_start, display);

    closeHeader(req);

    // ⚡ Bolt: Capture return value of snprintf to avoid O(N) strlen() call
    size_t out_len = (len > 0 && len < lock_page_size) ? len : lock_page_size - 1;
    esp_err_t out = httpd_resp_send(req, lock_page, out_len);
    free(lock_page);
    return out;
}