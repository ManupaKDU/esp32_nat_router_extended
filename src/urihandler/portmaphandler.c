...
const char *PORTMAP_ROW_TEMPLATE = "<tr> <th scope='row'>%s</th> <td>%hu</td> <td>%s</td> <td>%hu</td> <td> <form action='/portmap' method='POST' onsubmit=\"if(confirm('Are you sure you want to remove the portmap entry for external port %hu?')){ var b = this.querySelector('button'); b.classList.add('disabled'); b.setAttribute('aria-disabled', 'true'); return true; } return false;\"><input type='hidden' name='func' value='del'><input type='hidden' name='entry' value='%s'> <button title='Remove' name='remove' class='btn btn-light' aria-label='Remove portmap entry for external port %hu'> <svg version='2.0' width='16' height='16' aria-hidden='true'> <use href='#trash' /> </svg> </button> </form> </td></tr>";
