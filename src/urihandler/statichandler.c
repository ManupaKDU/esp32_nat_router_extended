...
esp_err_t download(httpd_req_t *req, const char *fileStart, size_t len)
{
    httpd_resp_set_hdr(req, "Cache-Control", "max-age=31536000");
    closeHeader(req);
    return httpd_resp_send(req, fileStart, len);
}
...
esp_err_t styles_download_get_handler(httpd_req_t *req)
{
    ...
    return download(req, (const char *)styles_start, (styles_end - styles_start) - 1);
}
...
esp_err_t jquery_get_handler(httpd_req_t *req)
{
    ...
    return download(req, (const char *)jquery_js_start, (jquery_js_end - jquery_js_start) - 1);
}
...
esp_err_t reset_get_handler(httpd_req_t *req)
{
    ...
    esp_err_t ret = httpd_resp_send(req, reset_start, (reset_end - reset_start) - 1);
    return ret;
}
