...
esp_err_t download(httpd_req_t *req, const char *fileStart, size_t fileSize)
{
...
esp_err_t styles_download_get_handler(httpd_req_t *req)
{
    ...
    const size_t styles_size = (styles_end - styles_start) - 1;
...
esp_err_t jquery_get_handler(httpd_req_t *req)
{
    ...
    const size_t jquery_size = (jquery_js_end - jquery_js_start) - 1;
    httpd_resp_set_type(req, "text/javascript");
    ESP_LOGD(TAG_HANDLER, "Requesting jquery");
    return download(req, (const char *)jquery_js_start, jquery_size);
}
...
esp_err_t reset_get_handler(httpd_req_t *req)
{
    ...
    const size_t reset_size = (reset_end - reset_start) - 1;

    closeHeader(req);

    esp_err_t ret = httpd_resp_send(req, reset_start, reset_size);
    return ret;
}
