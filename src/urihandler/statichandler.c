...
esp_err_t styles_download_get_handler(httpd_req_t *req)
{
    ...
    // ⚡ Bolt: Pass exact pre-calculated size instead of relying on HTTPD_RESP_USE_STRLEN implicit length check
...
esp_err_t redirectToRoot(httpd_req_t *req)
{
    ...
    httpd_resp_set_hdr(req, "Connection", "Close");
    // ⚡ Bolt: Avoid implicit strlen on empty string
...
esp_err_t reset_get_handler(httpd_req_t *req)
{
    ...
    const size_t reset_size = (reset_end - reset_start);
