...
esp_err_t download(httpd_req_t *req, const char *fileStart, size_t length)
{
    httpd_resp_set_hdr(req, "Cache-Control", "max-age=31536000");
    closeHeader(req);
    return httpd_resp_send(req, fileStart, length);
}
...
esp_err_t styles_download_get_handler(httpd_req_t *req)
{
    ...
    // ⚡ Bolt: Eliminate O(N) strlen() overhead by using linker symbols to calculate static asset size in O(1)
...
esp_err_t redirectToRoot(httpd_req_t *req)
{
    ...
    httpd_resp_set_hdr(req, "Connection", "Close");
    // ⚡ Bolt: Provide 0 explicitly to avoid O(N) strlen() call on an empty string
