...
## 2026-04-06 - Textual Loading Indicators
**Learning:** Pages involving long-running device operations, such as apply.html and scan.html, use <meta http-equiv="refresh"> tags to simulate waiting states. Because keyframe animations for standard Bootstrap spinners (spinner-border) are purged from the minified CSS, enhance these pages using textual visual loading indicators (e.g., ↻ Restarting...) instead of non-functional CSS spinners.
**Action:** When adding loading states to pages that rely on meta refresh tags for polling, use textual indicators (like `↻ Restarting...`) styled with existing classes (like `text-secondary`) instead of attempting to use CSS spinners whose animations have been stripped out.
