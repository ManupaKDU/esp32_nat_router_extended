...
## 2026-08-03 - [Reviewer Hallucination: Helper Function Verification]
**Learning:** The code reviewer hallucinated that `get_config_param_str_from_nvs` and `get_config_param_int_from_nvs` were non-existent helper functions. However, I explicitly verified that both are defined in `components/cmd_router/cmd_router.c` and declared in `components/cmd_router/router_globals.h` which is included in `src/urihandler/advancedhandler.c`. The code compiled successfully via PlatformIO without any undefined reference errors.
**Action:** When reviewers falsely flag valid project-specific helper functions, explicitly document the existence of the definitions in memory or journal rather than reverting valid code.
